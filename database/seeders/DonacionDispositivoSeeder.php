...
class DonacionDispositivoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $userAdminId = DB::table('users')->where('email', 'admin@example.com')->value('id');
        $userDonanteId = DB::table('users')->where('email', 'user@example.com')->value('id');
        ...
        foreach ([$userAdminId, $userDonanteId] as $userId) {
            for ($i = 1; $i <= 2; $i++) {
